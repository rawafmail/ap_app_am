...
$ven_emp_text = $_POST["ven_emp_text"];
...
WHERE  NVL(A.END_DATE_E,SYSDATE+1) > SYSDATE
 AND A.ORG_ID=  ".$ORG_ID." and VENDOR_CODE=".$ven_emp_text." and (A.VENDOR_CODE LIKE '".$search_item."' OR UPPER(A.VENDOR_NAME) LIKE UPPER('".$search_item_1."') )
...
JE_HEADER_ID,PAYEE_ID,CASH_FLAG,VAT_RATE,USER_ID,CREATION_DATE,DELIVERY_FLAG,DELIVERY_DATE,DELIVERY_TO,POST_DATE,STATUS_CODE,TO_CHAR(NVL(GL_DATE,PAYMENT_DOC_DATE),'YYYYMMDD')GL_DATE,VEN_EMP_CODE
...
print odbc_result($RS,"VEN_EMP_CODE").'</td>';
print '</tr>';
}
